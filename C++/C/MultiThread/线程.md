...
![image-20210718150521136](线程.assets/image-20210718150521136.png)	

结果不会断续输出，是因为io本身是可重入的，写入输出缓冲区会枷锁。不可冲入的函数会在名字上体现出来。如puts_unlocked()



### 5.8 线程与信号

每个线程都有自己的mask位图和pending位图，以进程为单位只有pending位图。当前从kernel扎回来user态调度的线程来响应进程的信号。
...
openMP实现并发的方式，提供一些语法标识由gcc编译器实现。

![image-20210718153252943](线程.assets/image-20210718153252943.png)	

cpu是几核就开启几个线程

![image-20210718154735401](线程.assets/image-20210718154735401.png)	
