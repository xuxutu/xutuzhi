...
```
1.每隔一秒响应一回，相当于alarm链
	struct itimerval itv;
 75     itv.it_interval.tv_sec = 1;
 76     itv.it_interval.tv_usec = 0;
 77     itv.it_value.tv_sec = 1;
 78     itv.it_value.tv_usec = 0;
  81     setitimer(ITIMER_REAL, &itv, NULL);
```
...
```
 ...
 69     sa.sa_sigaction = alrm_action;
 70     sigemptyset(&sa.sa_mask);
 71     sa.sa_flags = SA_SIGINFO;//当设定了该标志位时，表示信号附带的参数可以被传递到信号处理函数中
sigaction(SIGALRM, &sa, &alrm_sa_save);//注册信号，注册新的行为返回旧的行为

alrm_sa_save为全局变量
21 static struct sigaction alrm_sa_save;

```

```
3.atexit(module_unload);
```

###### 3.信号处理函数 void alrm_action

```
 28     if(info->si_code != SI_KERNEL)	//判断信号源，选择是否响应
 29         return;
 30     
 31     for(int i = 0; i < MYTBF_MAX; i++)	//累计token
 32     {
 33         if(job[i] != NULL)
 34         {
 35             job[i]->token += job[i]->cps;
 36             if(job[i]->token > job[i]->burst)
 37                 job[i]->token = job[i]->burst;
 38         }
 39     }
...
```
sfd = open(argv[1], O_RDONLY);
size = mytbf_fetchtoken(tbf, BUFSIZE);//获取期望值与实际值间的最小值
```

###### 5. mytbf_destroy //释放yoken

```
169     struct mytbf_st* me = ptr;
170     job[me->pos] = NULL;
171     free(ptr);
```
...
6.module_unload//恢复旧的信号行为，释放所有token

```
 50     struct itimerval itv;
 51     itv.it_interval.tv_sec = 0;
 52     itv.it_interval.tv_usec = 0;
 53     itv.it_value.tv_sec = 0;
 54     itv.it_value.tv_usec = 0;
 55 
 56     sigaction(SIGALRM, &alrm_sa_save, NULL);
 57 
 58     setitimer(ITIMER_REAL, &itv, NULL);
 59 
 60     for(int i = 0; i < MYTBF_MAX; i++)
 61         free(job[i]);
```
...
signal响应多个信号时会发生重入现象，可用sigaction代替
