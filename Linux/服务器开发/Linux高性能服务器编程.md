...
3位标志字段的第一位保留，第二位DF（don，t Fragment）标识禁止分片。如果设置这个位为1，IP模块将不会对数据报分片。在这种情况下，如果IP数据报长度超过MTU，IP模块将会丢弃这个数据报并返回一个ICMP差错报文。第三位MF（more fragment）标识是否是最后一个分片。

13位片偏移表示分片相对原始IP数据报开始处的偏移。实际的值是该值左移三位得到的。由于这个原因除了最后一个IP分片之外，每个IP分片的数据部分的长度必须是8的整数倍。保证后边的IP分片有一个合适的偏移值。

8位生存时间是数据报到达目的地之前允许经过的路由器跳数，TTL被发送端设置,一般为64。数据在转发的过程当中每经过一个路由，该值就被路由器减1。当TTL值为0时，路由器将丢弃数据报，并向源端发送一个ICMP差错报文。TTL值可以防止数据报陷入数据循环。

8位协议用来区分上层协议

![image-20210715103706230](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715103706230.png)

16位头部校验和由发送端填充，接收端对其使用CRC算法以校验IP数据报头部在传输过程中是否损坏。

32位源端和目的端地址标识了数据的发送端和接收端，一般这两个数据在传输过程中不会变，无论经历了多少个下一跳。

![](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715122918452.png)	
...
![image-20210715124245153](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715124245153.png)

### 2.3 IP分片

![](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715125800675.png)	

```
sudo tcpdump -ntv -i eno1 icmp //只抓取icmp
```

![image-20210715130404499](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715130404499.png)	

```
ping 10.74.185.196 -s 1473 //-s 指定每次发送1473个字节
```

![image-20210715130538546](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715130538546.png)	

最后IP层传给数据链路层的数据可能是**IP数据报**也可能是**IP分组**。



### 2.4 IP路由

IP协议的核心任务就是数据报的路由，即决定发送数据报到目标机器的路径。

#### 2.4.1 IP模块工作流程

![image-20210715131458063](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715131458063.png)

图2-3的虚线箭头显示了路由表的更新过程。这一过程指通过路由协议或route命令调整路由表，使之适应最新的网络拓扑结构，称为IP路由策略。

#### 2.4.2 路由机制

查看路由表

![image-20210715132514512](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715132514512.png)	

该路由项包含两项，每项包含八个字段

![image-20210715132831487](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715132831487.png)	

给定数据报的IP地址，路由过程：

1.查找路由表中和数据报的目标IP地址完全匹配的主机IP地址，如果找到就使用该路由项，如果没找到，则步骤2

2.查找路由表中和数据报的目标IP地址具有相同网络ID的网络IP地址，如果没找到，则步骤3

3.选择默认路由项，这通常意味着数据的下一跳路由是网关。
...
```
sudo route add -host 10.74.185.196 dev eno1 //所有到196的IP数据报将通过网卡eno1直接发送到目标主机的接收网卡
```

![image-20210715135419003](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715135419003.png)	

```
sudo route del -net 10.74.185.0 netmask 255.255.255.0//只能访问196
```

![image-20210715140053348](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715140053348.png)	

```
sudo route del default \\无法访问因特网
```

![image-20210715141848844](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715141848844.png)	



```
sudo route add default gw 10.74.185.196 dev eno1
```

![image-20210715141959613](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715141959613.png)	



```
sudo route add default gw 10.74.185.253 //设置默认网关
```

![](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210715143913679.png)	
...
**路由器能执行数据报的转发操作，而主机一般只接收和发送数据报**，这是因为主机上/proc/sys/net/ipv4/ip_forward 内核参数默认被设置为0。我们可以通过它来使主机转发数据报。

![image-20210729124247502](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729124247502.png)	
...
3.必要时，发送一个ICMP重定向报文，告诉他更合理的下一跳路由器
...
![image-20210729124908943](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729124908943.png)	
...
接收主机可以根据以上信息来判断由哪个路由器来转发，并更新路由表。通常是更新缓冲，而不是路由表。

决定是否允许发送ICMP报文的内核参数：

![image-20210729125708893](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729125708893.png)	

决定是否允许接收MP报文的内核参数：

![image-20210729125838671](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729125838671.png)	

一般来说，主机只能接收**ICMP重定向报文**，而路由器只能发送**ICMP重定向报文**
...
![image-20210729141009653](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729141009653.png)

129：

![image-20210729135232064](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729135232064.png)	

146：

![](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729135154138.png)	



![image-20210729135934887](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729135934887.png)	



129将数据报发送146（默认网关为146，146返回一个ICMP报文（一般主机不处理数据报，这里开启了数据报转发功能），告诉129更合理的下一跳。129更新路由缓存，通过获取的下一跳访问网络。

实际：

![image-20210729141359303](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729141359303.png)

第一个数据报将由146转发，后续通过更合理的网关访问。
...
![image-20210729142204884](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729142204884.png)	

4为版本号，指明IP协议的版本。IPV6，6

8位通信类型指明优先级

20位流标签，新增加字段，用于对连接服务的质量由特殊要求的通信

16位净荷长度除了固定头部的长度

8位下一个包头，指出固定头部后的包头类型

8位跳数限制，TTL

16字节来表示IP地址

![](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729142943245.png)	



#### 2.7.2 IPV6扩展头部

![image-20210729143121368](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729143121368.png)	
...
字节流服务和数据报服务的区别在应用层的的体现是通信双方是否执行相同的读写次数。当发送端连续执行多次写操作时，TCP模块先将这些数据放入TCP发送缓冲区。当TCP模块开始真正发送数据时，发送缓冲区的数据可能会被封装为一个或多个TCP报文段发出。

当接收端接收到一个或多个TCP报文段后，TCP模块将它们携带的应用程序数据按照TCP报文段的序号依次放入TCP接收缓冲区内。应用程序可以一次读取也可以分几次读取，取决于用户缓冲区的大小。这就是字节流的概念：应用程序对数据的发送和接收是没有边界限制的。

UDP模块将其封装成一个UDP报发送，接收端必须及时对数据包进行读操作，否则就会丢包。并且，如果应用程序如果没有指定足够的应用程序缓冲区，数据将会被截断。



图3-1和图3-2显示了TCP字节流服务和UDP数据报服务的上述区别。两图中省略了传输层以下的细节。

![image-20210729150313159](Linux%E9%AB%98%E6%80%A7%E8%83%BD%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%BC%96%E7%A8%8B.assets/image-20210729150313159.png)	

TCP传输是可靠的。首先，TCP协议采用发送应答机制，即发送端发送的每个TCP报文段都必须得到接收端的回答，才认为这个TCP报文段传输成功。其次，TCP采用超时重传机制，发送端在发送出一个TCP报文段之后启动定时器，如果规定时间内未收到应答，将重新发送。TCp将整理乱序的IP数据报最终交付给应用程序。

UDP协议则和IP协议一样，提供不可靠服务/它门需要上层协议来处理数据确认和超时重传。
